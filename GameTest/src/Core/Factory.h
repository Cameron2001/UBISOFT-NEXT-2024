...
{
public:
    static Entity createPlayer(Registry& registry, vec2 pos, float radius, int segments = 10, float health = 100.0f, float shieldHealth = 50.0f,  float speed = 1000.0f, float projForce = 20000.0f, float projHealth = 100.0f, float projDmg = 50.0f, float projRadius = 20.0f, float armLength = 40.0f, float coolDown = 4.0f, Entity ID = NULL);
    ...
    static Entity createProjectile(Registry& registry,vec2 pos, float radius, int segments, float force, float angle, float health=20.0f, float damage=50.0f, Entity ID = NULL);
