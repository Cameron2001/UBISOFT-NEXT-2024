...
void SArm::update(Registry& registry)
{
    for (const Entity ID : registry.getEntities<CArm>())
    {
        ...
        if (arm.state == CArm::ArmState::SHOOTING && !arm.bShoot)
        {
            ...
            factory::createProjectile(registry, armStart, arm.projectileRadius * multipler, circle.radius, 10,
                                      arm.projectileForce * multipler, arm.rotation, arm.projectileHealth * multipler,
                                      arm.projectileDamage * multipler);
