...
void SPlayer::update(Registry& registry, float dt)
{
    for(const Entity ID : registry.getEntities<CPlayer>())
    {
        ...
        if (App::IsKeyPressed('S'))rigidbody.acceleration = rigidbody.acceleration - vec2{0.0f,player.moveSpeed}; //DOWN
        if (App::IsKeyPressed('D'))rigidbody.acceleration = rigidbody.acceleration + vec2{player.moveSpeed,0.0f}; //RIGHT
        if (App::IsKeyPressed('A')) rigidbody.acceleration = rigidbody.acceleration - vec2{player.moveSpeed,0.0f}; //LEFT
